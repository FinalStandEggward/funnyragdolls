...
local SOUND_ID = Config.SOUND_ID
local IMAGE_ID = Config.IMAGE_ID

local BLOOD_COLOR = Config.BLOOD_COLOR
local BLOOD_LIFETIME = Config.BLOOD_LIFETIME
local BLOOD_EXPAND_TIME = Config.BLOOD_EXPAND_TIME
local BLOOD_COOLDOWN = Config.BLOOD_COOLDOWN
local BLOOD_MIN_PARTS = Config.BLOOD_MIN_PARTS
local BLOOD_MAX_PARTS = Config.BLOOD_MAX_PARTS
local BLOOD_MIN_SIZE = Config.BLOOD_MIN_SIZE
local BLOOD_MAX_SIZE = Config.BLOOD_MAX_SIZE
local BLOOD_SPREAD = Config.BLOOD_SPREAD
local BLOOD_SURFACE_OFFSET = Config.BLOOD_SURFACE_OFFSET

local bloodCooldown = {}

local function createBloodPixel(position, normal, finalSize, rotation)
    local blood = Instance.new("Part")

    blood.Name = "PixelBlood"
    blood.Anchored = true
    ...
    local surfacePosition =
        position + normal * BLOOD_SURFACE_OFFSET
...
local function spawnPixelSplatter(position, normal)
    ...
    for i = 1, amount do
        ...
        local randomZ = math.random(-100, 100) / 100
        ...
        local targetPosition =
            position + spreadOffset + normal * 0.5

        local rayOrigin = targetPosition
        local rayDirection = -normal * 2
...
local function setupBloodTouch(part)
    ...
    part.Touched:Connect(function(hit)
        ...
        if part.Parent and hit:IsDescendantOf(part.Parent) then
            return
        end
...
local function gibRagdoll(model)
    ...
    soundPart.Position = explosionPosition
    ...
    -- TF2-style critical hit image
    local effectPart =
        Instance.new("Part")

    effectPart.Name = "CriticalHitEffect"
    ...
    effectPart.Position = explosionPosition
    effectPart.Parent = Workspace

    local billboard =
        Instance.new("BillboardGui")

    billboard.Name = "CriticalHit"
    billboard.Adornee = effectPart
    billboard.AlwaysOnTop = false
    billboard.LightInfluence = 0
    billboard.Size = UDim2.fromOffset(250, 250)
...
-- Existing ragdolls
for _, ragdoll in ipairs(
    RagdollFolder:GetChildren()
) do
    task.spawn(gibRagdoll, ragdoll)
end

-- New ragdolls
RagdollFolder.ChildAdded:Connect(function(child)
    task.spawn(gibRagdoll, child)
end)
