--[[
    VAN RAGDOLL REST STOP v6

    Based on the original working Van Player admin command/model.
    The original sequence anchored the victim and moved its root between
    the van's internal TPPART points instead of applying a large physics impulse.
    This version keeps that idea, but makes the movement visible and deterministic.

    Sequence:
      1. Van spawns from a random cardinal direction.
      2. It approaches the ragdoll and stops beside it.
      3. Door opens and the van pauses.
      4. Ragdoll is visibly tossed through the door.
      5. Ragdoll is held inside while the van pauses.
      6. Door closes.
      7. Van continues in the SAME direction it arrived from.
      8. 20% chance to explode while driving away.
]]
...
local function createVan()
    local o1, o2, o3, o4, o5, o6, o7, o8, o9, o10, o11, o12, o13, o14, o15, o16, o17, o18, o19, o20, o21, o22, o23, o24, o25, o26, o27, o28, o29, o30, o31, o32, o33, o34, o35, o36, o37, o38, o39, o40, o41, o42, o43, o44, o45, o46, o47, o48, o49, o50, o51, o52, o53, o54, o55, o56, o57, o58, o59, o60, o61, o62, o63, o64, o65, o66, o67, o68, o69, o70, o71, o72, o73, o74, o75, o76, o77, o78, o79, o80, o81, o82, o83, o84, o85, o86, o87, o88, o89, o90, o91, o92, o93, o94, o95, o96, o97, o98, o99, o100, o101, o102, o103, o104, o105, o106, o107, o108, o109, o110, o111, o112, o113, o114, o115, o116, o117, o118, o119, o120, o121, o122, o123, o124
...
local function getDoor(van)
    return van:FindFirstChild("DOOR", true)
        or van:FindFirstChild("Door", true)
end

local function getVanPart(van, preferredNames)
    for _, name in ipairs(preferredNames) do
        local found = van:FindFirstChild(name, true)
...
local function setVanCollision(van)
    for _, obj in ipairs(van:GetDescendants()) do
        if obj:IsA("BasePart") then
            ...
            obj.CanQuery = true
            obj.Massless = true
            obj.CollisionGroup = VAN_GROUP
        end
    end
end

local function setPassengerCollision(ragdoll)
    for _, obj in ipairs(ragdoll:GetDescendants()) do
        if obj:IsA("BasePart") then
            obj.CollisionGroup = PASSENGER_GROUP
            obj.CanCollide = true
            obj.CanTouch = false
            obj.CanQuery = true
...
local function anchorPassenger(ragdoll, anchored)
    for _, obj in ipairs(ragdoll:GetDescendants()) do
        if obj:IsA("BasePart") then
...
-- The original model's driving direction is LOCAL -X.
-- Its side door is on LOCAL -Z.
-- Mapping those axes lets the exact same model work from all four
-- cardinal approach directions without making it drive sideways.
local function getOrientation(forward)
    local up = Vector3.yAxis
    local sideOut = forward:Cross(up).Unit

    -- local -X -> forward
    -- local -Z -> sideOut (the door faces the ragdoll)
    local worldX = -forward
    local worldZ = -sideOut

    return CFrame.fromMatrix(Vector3.zero, worldX, up, worldZ), sideOut
end

local function detachDoor(van, door)
    if not door then
        return
    end
...
local function moveModel(model, targetPivot, duration)
...
local function moveDoor(door, delta, duration)
...
local function playVanSound(van, soundId, volume)
...
--==================================================
-- VISIBLE RAGDOLL THROW
--==================================================

local function throwRagdollIntoVan(ragdoll, door, sideOut, van)
    if not ragdoll or not ragdoll.Parent or not van or not van.Parent then
        return false
    end

    local tpPart = van:FindFirstChild("TPPART", true)
    if not tpPart or not tpPart:IsA("BasePart") then
        warn("[Van Ragdolls] TPPART was not found inside the van.")
        return false
    end

    -- The van is parked from this torso position, so the ragdoll remains in
    -- its natural landing spot beside the door; it is not teleported outward.
    local root = ragdoll:FindFirstChild("HumanoidRootPart")
        or ragdoll:FindFirstChild("UpperTorso")
        or ragdoll:FindFirstChild("Torso")
        or ragdoll:FindFirstChild("LowerTorso")

    if not root or not root:IsA("BasePart") then
        warn("[Van Ragdolls] No usable torso/root was found for the throw.")
        return false
    end

    setPassengerCollision(ragdoll)
    ...
    -- Keep all limbs physically active for the toss.
    anchorPassenger(ragdoll, false)

    -- A single torso catcher guides the strong throw to TPPART. No movers are
    -- attached to limbs, so the existing ragdoll constraints can stay loose.
    local rootAttachment = Instance.new("Attachment")
    rootAttachment.Name = "VanThrowRootAttachment"
    rootAttachment.Parent = root

    local targetAttachment = Instance.new("Attachment")
    targetAttachment.Name = "VanThrowTargetAttachment"
    targetAttachment.Parent = tpPart

    local catcher = Instance.new("AlignPosition")
    catcher.Name = "VanThrowCatcher"
    catcher.Attachment0 = rootAttachment
    catcher.Attachment1 = targetAttachment
    catcher.ApplyAtCenterOfMass = true
    catcher.RigidityEnabled = false
    catcher.ReactionForceEnabled = false
    catcher.MaxForce = 35000
    catcher.MaxVelocity = 90
    catcher.Responsiveness = 9
    catcher.Parent = root

    local displacement = tpPart.Position - root.Position
    local throwDirection = displacement.Magnitude > 0.01 and displacement.Unit or -sideOut

    local THROW_SPEED = 56
    local UPWARD_POP = 6
    local SPIN_STRENGTH = 11
    local MAX_THROW_TIME = 0.90
    local CATCH_DISTANCE = 1.20

    root.AssemblyLinearVelocity = (throwDirection * THROW_SPEED)
        + Vector3.new(0, UPWARD_POP, 0)

    root.AssemblyAngularVelocity = Vector3.new(
        math.random(-SPIN_STRENGTH, SPIN_STRENGTH),
        math.random(-SPIN_STRENGTH, SPIN_STRENGTH),
        math.random(-SPIN_STRENGTH, SPIN_STRENGTH)
    )

    local elapsed = 0
    while elapsed < MAX_THROW_TIME do
        if not ragdoll.Parent or not van.Parent or not tpPart.Parent or not root.Parent then
            break
        end
        elapsed += RunService.Heartbeat:Wait()
        if (root.Position - tpPart.Position).Magnitude <= CATCH_DISTANCE then
            break
        end
    end
    ...
    if not ragdoll.Parent or not van.Parent or not tpPart.Parent then
        return false
    end

    anchorPassenger(ragdoll, true)
    ragdoll:PivotTo(CFrame.new(tpPart.Position) * ragdoll:GetPivot().Rotation)
...
--==================================================
-- CARRY RAGDOLL WITH THE VAN
--==================================================

local function carryRagdoll(van, ragdoll, carryOffset)
    if not van or not ragdoll then
        return nil
    end

    local connection
    connection = RunService.Heartbeat:Connect(function()
        if not van.Parent or not ragdoll.Parent then
            if connection then
                connection:Disconnect()
            end
            return
        ...
        ragdoll:PivotTo(van:GetPivot() * carryOffset)
    end)
...
local function explodeVan(van, ragdoll)
    if not van or not van.Parent then
        return
    end
    ...
    local center = van:GetPivot().Position

    local explosion = Instance.new("Explosion")
    explosion.Position = center
    explosion.BlastRadius = EXPLOSION_RADIUS
    explosion.BlastPressure = EXPLOSION_PRESSURE
    explosion.DestroyJointRadiusPercent = 0
    explosion.ExplosionType = Enum.ExplosionType.NoCraters
    explosion.Parent = Workspace

    playVanSound(van, "rbxassetid://138983581", 3)
    ...
    for _, obj in ipairs(van:GetDescendants()) do
        if obj:IsA("BasePart") then
            ...
            local direction =
                (offset.Magnitude > 0.05 and offset.Unit or Vector3.new(
                    math.random() - 0.5,
                    1,
                    math.random() - 0.5
                ).Unit)
...
local function runKidnap(ragdoll)
    ...
    local success, err = pcall(function()
            ...
            if not ragdoll.Parent then
                return
            end

            local torso = ragdoll:FindFirstChild("HumanoidRootPart")
                or ragdoll:FindFirstChild("UpperTorso")
                or ragdoll:FindFirstChild("Torso")
                or ragdoll:FindFirstChild("LowerTorso")
                or ragdoll:FindFirstChildWhichIsA("BasePart")
            ...
            local ragdollPosition = torso.Position
            ...
            local forward = directions[math.random(1, #directions)]
            local rotation, sideOut = getOrientation(forward)
            ...
            -- Detach only the door's own joints so its animation is reliable.
            detachDoor(van, door)

            local basePivot = van:GetPivot()
            local doorLocal = basePivot:ToObjectSpace(door.CFrame)

            -- Align the side-door center directly to the settled torso. The
            -- pivot is derived from the door's local offset, keeping the van
            -- body sideways rather than centered on top of the ragdoll.
            local desiredDoorPosition =
                ragdollPosition
                + sideOut * PICKUP_SIDE_OFFSET
                + Vector3.new(0, 0.2 + VAN_HEIGHT_OFFSET, 0)

            local targetPivotPosition =
                desiredDoorPosition
                - rotation:VectorToWorldSpace(doorLocal.Position)

            local pickupPivot =
                CFrame.new(targetPivotPosition) * rotation

            -- Spawn 4 studs higher, then approach the pickup point.
            local spawnPivot =
                pickupPivot
                + Vector3.new(0, SPAWN_HEIGHT_OFFSET, 0)
                - forward * APPROACH_DISTANCE

            van:PivotTo(spawnPivot)

            local travelDistance =
                (van:GetPivot().Position - pickupPivot.Position).Magnitude

            moveModel(
                van,
                pickupPivot,
                math.max(travelDistance / DRIVE_SPEED, 0.1)
            )

            task.wait(ARRIVAL_PAUSE)

            -- Open: outward first, then slide along the van's local X axis.
            moveDoor(door, sideOut * DOOR_OUTWARD_DISTANCE, DOOR_STEP_TIME)

            local slideDirection = door.CFrame.RightVector * DOOR_SLIDE_DISTANCE
            moveDoor(door, slideDirection, DOOR_STEP_TIME * 4)

            task.wait(0.25)

            -- Throw the ragdoll physically toward TPPART.
            -- It is only anchored before/after the flight.
            local throwSuccess = throwRagdollIntoVan(
                ragdoll,
                door,
                sideOut,
                van
            ...
            -- Keep it visible inside while the van rests.
            local carryPivot = van:GetPivot()
            local carryOffset = carryPivot:ToObjectSpace(ragdoll:GetPivot())

            carryConnection = carryRagdoll(van, ragdoll, carryOffset)

            task.wait(INSIDE_PAUSE)

            -- Close door in exact reverse order.
            moveDoor(
                door,
                -slideDirection,
                DOOR_STEP_TIME * 4
            ...
            moveDoor(
                door,
                -sideOut * DOOR_OUTWARD_DISTANCE,
                DOOR_STEP_TIME
            )

            task.wait(DEPART_PAUSE)

            -- IMPORTANT: departure uses the SAME 'forward' vector.
            -- No reverse path and no turning around.
            local departPivot =
                van:GetPivot()
                + forward * DEPART_DISTANCE

            moveModel(
                van,
                departPivot,
                DEPART_DISTANCE / DEPART_SPEED
            )
